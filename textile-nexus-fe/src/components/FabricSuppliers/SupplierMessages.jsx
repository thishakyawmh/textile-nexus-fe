...
const SystemMessage = ({ text }) => (
    <div className="w-full bg-[#F3F4F6] dark:bg-neutral-800 p-4 rounded-xl text-xs text-neutral-600 dark:text-neutral-300 leading-relaxed max-w-3xl mx-auto my-4 relative transition-colors">
        {/* Avatar Placeholder */}
        <div className="absolute top-4 -left-12 w-8 h-8 rounded-full bg-neutral-200 dark:bg-neutral-700"></div>
        {text}
        <div className="absolute bottom-2 right-4 text-[10px] text-neutral-400">6.30 pm <MoreVertical size={10} className="inline ml-1" /></div>
    </div>
);


export default function SupplierMessages({ onNavigate }) {
            ...
            <main className="flex-1 flex flex-col h-screen overflow-hidden">
                ...
                <div className="flex-1 overflow-hidden p-6 lg:p-8 flex gap-6">
                    ...
                    <div className="w-64 shrink-0 hidden lg:flex flex-col h-full bg-white dark:bg-white/5 rounded-3xl p-6 shadow-sm border border-neutral-100 dark:border-white/10 transition-colors">
                        ...
                        <div className="mb-8">
                            <h3 className="text-xs font-bold text-neutral-900 dark:text-neutral-200 mb-4 px-2">My Email</h3>
                        ...
                        <div>
                            <h3 className="text-xs font-bold text-neutral-900 dark:text-neutral-200 mb-4 px-2">Label</h3>
                            <div className="space-y-3 px-3">
                                {MESSAGE_LABELS.map(label => (
                                    <div key={label.name} className="flex items-center gap-3 text-sm font-medium text-neutral-600 dark:text-neutral-400 cursor-pointer">
                                        <div className={cn("w-3 h-3 rounded border border-neutral-300 dark:border-neutral-600", "flex items-center justify-center")}>
                                            {/* Checkbox look */}
                                        </div>
                                        <span>{label.name}</span>
                                    </div>
                                ))}
                                <div className="flex items-center gap-2 text-xs font-medium text-neutral-400 mt-4 cursor-pointer hover:text-neutral-600">
                    ...
                    <div className="flex-1 flex flex-col h-full bg-white dark:bg-white/5 rounded-3xl shadow-sm border border-neutral-100 dark:border-white/10 overflow-hidden transition-colors">
                        ...
                        <div className="px-8 py-5 border-b border-neutral-100 dark:border-white/10 flex items-center justify-between bg-white dark:bg-white/5 shrink-0 transition-colors">
                            <div className="flex items-center gap-4">
                                <button className="p-1 hover:bg-neutral-50 dark:hover:bg-neutral-800 rounded-lg lg:hidden">
                                    <ChevronLeft size={20} />
                                </button>
                                <h3 className="text-lg font-bold text-neutral-900 dark:text-white">FashionTex</h3>
                                <div className="bg-purple-100 dark:bg-purple-900/20 text-purple-600 dark:text-purple-400 px-3 py-1 rounded text-[10px] font-bold uppercase tracking-wide">
                                    Factory
                                </div>
                            </div>
                            <div className="flex items-center gap-2">
                                <button className="p-2 text-neutral-400 dark:text-neutral-500 hover:text-neutral-600 dark:hover:text-neutral-300 hover:bg-neutral-50 dark:hover:bg-neutral-800 rounded-lg transition-colors">
                                    <Printer size={18} />
                                </button>
                                <button className="p-2 text-neutral-400 dark:text-neutral-500 hover:text-neutral-600 dark:hover:text-neutral-300 hover:bg-neutral-50 dark:hover:bg-neutral-800 rounded-lg transition-colors">
                                    <Star size={18} />
                        ...
                        <div className="flex-1 overflow-y-auto p-8 space-y-8 bg-white dark:bg-white/5 transition-colors">
                            ...
                            <SystemMessage
                                text={
                                    <>
                                        <p className="font-bold mb-1">This is FashionTex Factory. We are planning our January production batch and need the updated price list for Cotton, Linen, and Rayon fabrics.</p>
                                        <p>Please share the revised prices and minimum order quantities at your earliest convenience.</p>
                                    </>
                                }
                            />
                            ...
                            <MessageBubble
                                isSender={true}
                                time="6.34 pm"
                                text="Thank you for reaching out. Please find the updated prices below Cotton Plain: LKR 250 per meter , Linen Soft: LKR 680 per meter & Rayon Printed: LKR 520 per meter Minimum order quantity for all three fabrics is 50 meters. Let us know if you need samples or wish to proceed with an order."
                            />

                            <SystemMessage
                                text={
                                    <>
                                        <p>We have an upcoming bulk order and would like to confirm your current delivery timeline.</p>
                                        <p>Could you please share the estimated delivery duration for orders above 300 meters?</p>
                                        <p>We need this information before confirming our next purchase order.</p>
                                        <p>Thank you.</p>
                                    </>
                                }
                            />
                        ...
                        <div className="p-6 border-t border-neutral-100 dark:border-white/10 bg-white dark:bg-white/5 shrink-0 transition-colors">
                            <div className="flex items-center gap-4">
                                ...
                                <input
                                    ...
                                    className="flex-1 bg-transparent border-none outline-none text-sm text-neutral-700 dark:text-neutral-200 placeholder:text-neutral-300 dark:placeholder:text-neutral-600"
                                />
                                <div className="flex items-center gap-4">
                                    <button className="text-neutral-400 hover:text-neutral-600 dark:hover:text-neutral-300">
                                        <Paperclip size={20} />
                                    ...
                                    <button className="px-6 py-2 bg-black dark:bg-white text-white dark:text-black rounded-lg flex items-center gap-2 text-sm font-bold shadow-lg shadow-black/20 dark:shadow-none hover:bg-neutral-900 dark:hover:bg-neutral-200 transition-all">
